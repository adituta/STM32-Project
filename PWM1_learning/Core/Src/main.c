...
int main(void)
{
  ...
  send_message("Incepe exercitiul stm32_pwm1!\r\n", 32);
  ...
  // funcita de mai sus schimba duty cycle-ul din cod fata de schimbarea din HW pe care o fac in CubeMX
    ...
    /* USER CODE BEGIN 3 */
	  ...
	  now = HAL_GetTick();
	 	  ...
	 	  if(now>=next_tick){
	 		  ...
	 		  // vreau sa si trimit la a cata secunda sunt
	 		  int mess_len = snprintf(message_buffer, 100, "Impuls trimis la secunda: %lu | loop: %lu\r\n",now/1000, loop_count );

	 		  send_message(message_buffer, mess_len);
